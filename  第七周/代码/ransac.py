...
def test():
    ...
    #加入高斯噪声,最小二乘能很好的处理
    A_noisy = A_exact + np.random.normal(size = A_exact.shape ) #500 * 1行向量,代表Xi  自变量加噪声  https://www.cnblogs.com/luckyboy314/p/11715643.html
    B_noisy = B_exact + np.random.normal(size = B_exact.shape ) #500 * 1行向量,代表Yi  因变量加噪声
    ...
    if 1:
        ...
        sort_idxs = np.argsort(A_exact[:,0])    # max--->min indices ---Returns the indices that would sort an array.
        A_col0_sorted = A_exact[sort_idxs] # 秩为2的数组  排序之后直线
        ...
        pylab.plot( A_col0_sorted[:,0],
                    ...
                    label='RANSAC fit')
        pylab.plot( A_col0_sorted[:,0],
                    np.dot(A_col0_sorted,perfect_fit)[:,0], 'g',
                    label='exact system' )
        pylab.plot( A_col0_sorted[:,0],
                    np.dot(A_col0_sorted,linear_fit)[:,0], 'orange',
                    label='linear fit' )
        pylab.legend()
        pylab.show()
