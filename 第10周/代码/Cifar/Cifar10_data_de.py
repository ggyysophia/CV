...
#设定用于训练和评估的样本总数 , 样本标签总数
num_classes = 10
num_examples_pre_epoch_for_train = 50000
num_examples_pre_epoch_for_test = 50000


#定义一个空类，用于返回读取的Cifar-10的数据, 该类需要的属性在下面定义
class CIAR10Record(ogject):
    pass
...
def read_cifar10(file_queue):
    ...
    #使用tf.FixedLengthRecordReader()创建一个文件读取类。该类的目的就是读取文件  https://blog.csdn.net/fegang2002/article/details/83046584
...
def inputs(data_dir, batch_size, distorted):
    ...
    reshaped_image = tf.cast(read_input, tf.int32)

    num_exmples_per_epoch = num_examples_pre_epoch_for_train
    ...
    if distorted != None:
        ...
        fipped_image = tf.image.random_flip_left_right(cropped_image)
        #将左右翻转好的图片进行随机亮度调整，使用tf.image.random_brightness()函数
        adjuasted_brightness = tf.image.random_brightness(fipped_image, max_delta=0.8)
        #将亮度调整好的图片进行随机对比度调整，使用tf.image.random_contrast()函数
        adjuasted_contrast = tf.image.random_contrast(image=adjuasted_brightness, lower=0.2, upper=1.8)
        #进行标准化图片操作，tf.image.per_image_standardization()函数是对每一个像素减去平均值并除以像素方差
        float_image = tf.image.per_image_standardization(adjuasted_contrast)
        #设置图片数据及标签的形状
        float_image.set_shape([24, 24, 3])
        read_input.label.set_shape([1])

        min_queue_examples = int(num_examples_pre_epoch_for_test * 0.4)
        print('FIlling queue with %d CTFAR images before starting to train. This will take a few minutes'
              %min_queue_examples)

        #使用tf.train.shuffle_batch()函数随机产生一个batch的image和label
        image_train, labels_train = tf.train.shuffle_batch([float_image, read_input],batch_size=batch_size,
                                                           num_threads=16,
    ...
    #不对图像数据进行数据增强处理
        ...
        float_image.set_shape([24, 24, 3])
        read_input.set_shape([1])

        min_queue_examples = int(num_exmples_per_epoch * 0.4)
        #这里使用batch()函数代替tf.train.shuffle_batch()函数  备注
        image_test, labels_test = tf.train.shuffle_batch([float_image, read_input],
